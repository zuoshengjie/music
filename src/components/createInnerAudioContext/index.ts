import { createCallbackManager } from '@tarojs/taro-h5/src/api/utils';
  ...
  simpleEvents.forEach((eventName) => {
    simpleListenerTuples.forEach(([eventNamePrefix, listenerFunc]) => {
      Object.defineProperty(iac, `${eventNamePrefix}${eventName}`, {
        get() {
          return (callback) =>
            listenerFunc.call(audioEl, eventName.toLowerCase(), callback);
        },
      });
    });
  });
  ...
  customEvents.forEach((eventName) => {
    customListenerTuples.forEach(([eventNamePrefix, actionName]) => {
      Object.defineProperty(iac, `${eventNamePrefix}${eventName}`, {
        get() {
          return (callback) =>
            callbackManagers[eventName.toLowerCase()][actionName](callback);
        },
      });
    });
  });
